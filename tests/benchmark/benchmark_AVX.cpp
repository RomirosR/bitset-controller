...
template <size_t SZ> 
BitsetControllerAVX<SZ> RandomBitset(double lb = 0, double ub = 1) {
...
std::tuple<std::array<BitsetControllerAVX<B_SZ>, O_SZ>, std::array<int, O_SZ>, std::array<int, O_SZ>> GenData(double lb = 0, double ub = 1) {
    std::array<BitsetControllerAVX<B_SZ>, O_SZ> bc;
    for (auto& x : bc) {
        x = RandomBitset<B_SZ>(lb, ub);
    }
...
TEST_CASE("Find_next") {
    ...
    BENCHMARK_ADVANCED("[avx] Random data, iterate over ones, size = 65536, popcnt ~ 70-100%")(Catch::Benchmark::Chronometer meter) {
